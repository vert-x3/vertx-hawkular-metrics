...
an Hawkular Metrics service and the inventory structure generated will be sent to an Hawkular Inventory Service.
...
vertxOptions.setInventoryEnabled(true)
            .setFeedId("vertx-feed-id")
            .setVertxRootResourceId("root-resource-id");  // <8>


vertxHawkularOptions.addDisabledMetricsType(MetricsType.NET_SERVER); // <9>

VertxOptions vertxOptions = new VertxOptions()
        .setMetricsOptions(vertxHawkularOptions); // <10>

Vertx vertx = Vertx.vertx(vertxOptions); // <11>
----
...
<8> The *Inventory is disabled by default*, so enable it and provide a *feed id* and a *root resource id* if you want to use it.
<9> Disable unwanted metrics by adding the type of the metrics into a set, defaults to an empty set.
Available Types : `NET_SERVER`, `NET_CLIENT`, `HTTP_SERVER`, `HTTP_CLIENT`, `DATAGRAM_SOCKET`, `EVENT_BUS`.
<10> Add your `VertxHawkularOptions` to the `io.vertx.core.VertxOptions`
<11> Create the Vert.x instance
